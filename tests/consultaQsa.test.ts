...
/**
 * Função para testar a consulta de QSA pelo CNPJ
 * @param cnpj CNPJ a ser consultado
 */
async function testConsultaQsa(cnpj: string): Promise<void> {
  ...
  try {
    ...
    console.log(`Consultando QSA do CNPJ: ${cnpj} (${cnpjLimpo})`);
    ...
    console.error('Erro ao consultar QSA:');
    if (error.response) {
...
const cnpjTeste = '${CNPJ_TESTE_QSA}'; // CNPJ do Banco Bradesco

// Executa o teste
testConsultaQsa(cnpjTeste)
  .then(() => console.log('\nTeste concluído'))
