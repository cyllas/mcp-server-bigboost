...
/**
 * Função para testar a consulta de pessoa pelo CPF
 * @param cpf CPF a ser consultado
 */
async function testConsultaPessoa(cpf: string): Promise<void> {
  ...
  try {
    ...
    const payload = {
      q: `doc{${cpfLimpo}}`,
      Datasets: "basic_data"
      // Removendo o parâmetro view para testar
    };
    
    console.log(`Consultando CPF: ${cpf} (${cpfLimpo})`);
...
const cpfTeste = '${CPF_TESTE}';

// Executa o teste
testConsultaPessoa(cpfTeste)
  .then(() => console.log('\nTeste concluído'))
